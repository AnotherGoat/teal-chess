...
final class BishopMoveGeneratorTest {
    ...
    void normalMoves() {
        ...
        var generator = new BishopMoveGenerator(position);
        ...
        var unexpectedMoves = new Move[] {
                new Move(NORMAL, d4, g7),
                new Move(NORMAL, d4, b6)
        };

        var captures = 1;
        assertThat(generator.generate()).hasSize(captures + 9).containsOnlyOnce(expectedMoves).doesNotContain(unexpectedMoves);
    }
    ...
    void captures() {
        ...
        var expectedCaptures = new Move[] {
                new Move(CAPTURE, d4, a7),
                new Move(CAPTURE, d4, b2),
                new Move(CAPTURE, d4, e3)
        };

        var unexpectedCaptures = new Move[] {
                new Move(CAPTURE, d4, e5),
                new Move(CAPTURE, d4, c3)
        };
    ...
    void blockedSlidingMoves() {
        ...
        var unexpectedMoves = new Move[] {
                new Move(NORMAL, e4, h7),
                new Move(NORMAL, e4, a8),
                new Move(CAPTURE, e4, c2),
                new Move(CAPTURE, e4, g2)
        };
