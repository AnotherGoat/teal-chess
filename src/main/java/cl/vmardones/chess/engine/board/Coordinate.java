...
/**
 * A coordinate is one of the 64 positions where a chess piece can be.
 * It's usually identified by the chess algebraic notation, which consists of the coordinate's rank (a-h) folowed by its column (1-8).
 */
public final class Coordinate {
    ...
    /**
     * Get the array index of this coordinate.
     * Indexes start at 0 in the a8 coordinate, and end at 63 in the h1 coordinate.
    ...
    private static int calculateIndex(final String algebraicCoordinate) {
        ...
        final var rank =
                Board.NUMBER_OF_RANKS * (Board.NUMBER_OF_RANKS - Integer.parseInt("" + algebraicCoordinate.charAt(1)));

        return column + rank;
    }

    /**
     * Obtains the rank (row in chess terminology) of this coordinate.
     * The rank is a number between 1 (bottom of the board, white side) and 8 (top of the board, black side).
    ...
    /**
     * Obtains the column of this coordinate.
     * The column is a letter between lowercase a (left side or queen's side) and lowercase h (right side or king's side).
    ...
    /**
     * Obtains the column of this coordinate as a number.
     * The a column's index is 0 and the h column's index is 8.
    ...
    public Optional<Coordinate> to(final int x, final int y) {
        try {
            final var destination = COORDINATES_CACHE.get(index + horizontalClamp(x) - y * Board.NUMBER_OF_RANKS);
