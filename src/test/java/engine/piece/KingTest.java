...
class KingTest {
  ...
  @Test
  void diagonalMove() {
    assertThat(Arrays.asList(king.getMoveOffsets()).contains(new int[] {1, 1})).isTrue();
  }

  @Test
  void horizontalMove() {
    assertThat(Arrays.asList(king.getMoveOffsets()).contains(new int[] {0, 1})).isTrue();
  }

  @Test
  void verticalMove() {
    assertThat(Arrays.asList(king.getMoveOffsets()).contains(new int[] {1, 0})).isTrue();
  }

  @Test
  void illegalMove() {
    assertThat(Arrays.asList(king.getMoveOffsets()).contains(new int[] {2, 2})).isFalse();
  }
